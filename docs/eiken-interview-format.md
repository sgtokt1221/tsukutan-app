...
問題を選ぶ画面は1枚目のイラストを出す。出すのは `public/eiken/thumbs/` に置く
480px幅の写し（約20KB）。本番用の1400px を5枚並べると、選ぶだけで1MB近く落ちる。

カードを足したら作り直すこと。`index.json` の `thumbnail` も同時に書かれる。

```bash
node scripts/buildInterviewThumbs.js --write   # cwebp が要る
```
