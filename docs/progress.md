...
| 指標 | 基準（フェーズ0） | フェーズ4後 | フェーズ5後 | フェーズ6後 | フェーズ7後 |
|---|---|---|---|---|---|
| `npm run build` | 成功・警告あり | 成功・警告1件 | 成功・警告1件 | 成功・警告1件 | 成功・警告1件 |
| `npm run lint` | **スクリプト未定義** | 警告1件・エラー0 | 警告1件・エラー0 | 警告1件・エラー0 | 警告1件・エラー0 |
| JSバンドル（gzip） | 766.17 kB | 822.1 kB（IDを付けた分。フェーズ8で除去） | 822.93 kB | 821.4 kB | 821.54 kB |
| ビルドの想定ホスト | `/tsukutan-app/` | `/` | `/` | `/` | `/` |
| アプリ側 `npm test` | 0件のため失敗 | 48件 成功 | **71件 成功** | **113件 成功** | **132件 成功** |
| Functions ユニットテスト | なし | 26件 成功 | 26件 成功 | 26件 成功 | 26件 成功 |
| Rules 許可・拒否テスト | なし | 21件 成功 | 21件 成功 | 21件 成功 | 21件 成功 |
| アプリ依存監査 | 69件 / critical 4 | 変化なし | 変化なし | 変化なし | 変化なし |
...
| Firestore Rules | **バージョン管理外** | 管理下 | 管理下 | 管理下 | 管理下 |
| 設定の不整合（`check-config-consistency`） | 目標ID 8種の不整合 | 不整合なし | 不整合なし | 不整合なし | 不整合なし |
| `MOTIVATION_LEVELS` の定義箇所 | 3ファイルに重複 | 1ファイル | 1ファイル | 1ファイル | 1ファイル |
| `src/App.js` | 370行 | 153行 | 153行 | 153行 | 153行 |
| 単語の永続ID | なし（配列index） | **6,736件すべてに付与** | 6,736件 | 6,736件 | 6,736件 |
| `src/VocabularyCheckTest.js` | 975行 | 975行 | 975行 | **302行** | 302行 |
| 5問目でのステージリセット | **発生する** | 発生する | 発生する | **しない** | しない |
| スクリプトのテスト | なし | **13件 成功** | 13件 成功 | 13件 成功 | 13件 成功 |
| やる気レベルが日次計画に反映される | **されない** | されない | **される** | される | される |
| 隣接レベル語の出題 | **常に0件（沈黙失敗）** | 同左 | **動作** | 動作 | 動作 |
| 管理画面のストーリー表示 | **常に空（沈黙失敗）** | 同左 | 同左 | 同左 | **表示される** |
| `dangerouslySetInnerHTML` | あり | あり | あり | あり | **なし** |
| 未使用単語の2本目生成 | **必ず429で失敗** | 同左 | 同左 | 同左 | **サーバー内で1回で完結** |
...
## フェーズ6: 単語力チェックの再構築（完了）

### 見つかった不具合

**1. 5問目でステージが作り直され、得点も回答時間も消えていた。**
...
useEffect(() => {
  ...
  if (allWords.length > 0) setupStage(adaptiveLevel);
...
if (newPerformanceHistory.length % 5 === 0) {
  const newAdaptiveLevel = calculateAdaptiveLevel(...);
  if (newAdaptiveLevel !== adaptiveLevel) setAdaptiveLevel(newAdaptiveLevel);
}
```

があり、**5問目で難易度が動くと `adaptiveLevel` が変わって useEffect が再実行され、
そのステージが最初から作り直されていた。** 同時に `evaluateStage(newScore)` も走るので、
どちらが勝つかはタイミング次第。計画書11.6の最初のテスト項目そのもの。

**2. `pre1` / `pre2` を引き算していた。**

```js
return word.eikenLevels.some(eikenLevel => Math.abs(eikenLevel - effectiveLevel) <= 1);
```

`eikenLevels` には `'pre1'` / `'pre2'` という文字列が入る。
`'pre1' - 3` は `NaN`、`Math.abs(NaN) <= 1` は `false` なので、
**準1級・準2級の単語が出題候補から静かに落ちていた。**
`calculateWordDifficulty` も文字列を 0 に潰していた。

**3. レベル体系が混ざっていた。**

同じ `currentLevel` 変数を、単語のレベル（大きいほど難しい）と
英検の級（大きいほど易しい）の両方として使っていた。同じ関数の中に

```js
nextLevel = Math.min(7, currentLevel + 1);   // 合格 → レベルアップ
if (currentLevel === 4) nextLevel = 5;       // 4級で失敗 → 5級（易しい方）へ
```

が同居していて、方向が逆。

**4. 出題済みの単語を管理していなかった。** 同じテスト中に同じ単語が再度出うる。

**5. ステージ得点と全体得点が混ざっていた。** 早期終了の判定に使う
`totalQuestions` はステージ数から逆算した推定値で、実際の回答数と一致しない。

### 作り直し

`src/logic/placementTestEngine.js` に判定を切り出した。React には一切依存しない。

| 規則 | 実装 |
|---|---|
| 出題は単語の `level`（1〜7）だけで選ぶ | `eikenLevels` は engine で一切参照しない |
| 難易度が動くのはステージを締めたときだけ | `completeStage` の中でのみ `targetLevel` が変わる |
| ステージ途中で何も初期化しない | `recordAnswer` は履歴に積むだけ |
| 同じ単語を二度出さない | `askedIds` を持ち、`selectQuestions` で除外 |
| ステージ得点と全体得点を分ける | `stageAnswers` / `allAnswers` |
| 早期終了は最低回答数を満たしてから | 15問以上 かつ レベルが2ステージ動かない |
| 同じ履歴なら同じ結果 | すべて純粋関数 |
| 出題順を再現できる | `selectQuestions(words, level, count, askedIds, random)` に乱数を注入 |

`VocabularyCheckTest.js` は **975行 → 302行**。
判定を全部エンジンへ渡し、残ったのは表示と入力だけ。

そのほか変えたこと:

- 推定語彙数を永続IDのユニーク件数から出す（同じ単語が複数教材にあっても二重に数えない）
- **保存に失敗したら完了画面へ進まず、再試行ボタンを出す**（以前は alert のみ）
- スワイプできない環境向けに「わかる」「わからない」ボタンを追加（最小タップ領域44px）
- 判定完了後は出題画面を畳む。回答ボタンが残っていると二重保存になる
- 前の問題へ戻ると直前の回答は取り消すが、出題済みからは外さない（再出題しないため）

### テスト

| ファイル | 件数 | 内容 |
|---|---:|---|
| `src/logic/placementTestEngine.test.js` | 28 | ステージ進行・レベル判定・早期終了・pre1/pre2・乱数注入・戻る操作 |
| `src/VocabularyCheckTest.test.js` | 14 | 画面としての動作（ステージが巻き戻らない、重複出題なし、保存失敗時の扱い） |

計画書11.6の必須テストはすべて入っている。アプリ側は合計 **113件**。

コンポーネントテストで実際にボタンを押して画面遷移を確認しているので、
「5問目でステージがリセットされない」ことは画面レベルで検証済み。

### 未確認

- 実ブラウザでのスワイプ操作は未確認（生徒ログインが必要）。
  ボタン操作はコンポーネントテストで検証済み。
- 早期終了の閾値（15問 / 2ステージ安定 / 70%・30%）は既存実装から引き継いだ
  値ではなく、こちらで決め直した。運用しながら調整してほしい。

## フェーズ7: AIストーリー機能（完了 / 未デプロイ）

### 見つかった不具合

**1. 未使用単語の作り直しは必ず429で失敗していた。**

クライアントは1回目の結果に `unusedWords` があると、同じエンドポイントを2回目に呼んでいた。
しかしサーバーは1回目の時点で `generatedStories/{YYYY-MM}` を作るので、
2回目は月次制限に引っかかって **必ず 429 を返す**。
つまり「使えなかった単語の2本目」は一度も作られていない。

**2. 保存済みストーリーが読み込めていなかった。**

サーバーが保存するのは `{ story, translation, words, unusedWords }`。
一方クライアントの読み込みは
...
を見ていた。**`story1` は存在しないので `sentences` が空**になり、
リロードすると本文が消える。

**3. 管理画面のストーリーが常に空だった。**（フェーズ0の監査で検出）

書き込みと生徒画面は `generatedStories`、管理画面だけ `stories` を読んでいた。

**4. 同時クリックで二重生成できた。** 存在チェックと書き込みが別トランザクション。

**5. AI生成文をHTMLとして実行していた。**
...
生成された文章に HTML が混ざれば、そのままブラウザが解釈する。
...
認証確認
  ↓
月次ドキュメントをトランザクションで予約（status: generating）
  ├─ complete / 旧スキーマ → 429 で既存を返す
  ├─ generating かつ5分以内 → 409「生成中です」
  └─ failed / 期限切れ → 上書きして再挑戦
  ↓
1回目のストーリー生成 → 翻訳
  ↓
未使用単語があれば、同じ Functions 実行の中で2回目 → 翻訳
  ↓
status: complete で保存
  ↓
失敗したら status: failed と理由・時刻を残す
```
...
```json
{
  "status": "complete",
  "title": "今月の長文",
  "sentences": [{ "english": "...", "japanese": "..." }],
  "usedWordIds": [], "unusedWordIds": [],
  "usedWords": [], "unusedWords": [],
...
- API 呼び出しを1回に。2回目の呼び出しを削除
- 読み込みは `generatedStories` だけ。**管理画面も `generatedStories` に変更**
- 旧スキーマ（`story1`/`story2`、および `story`/`translation`）は読み込み時にだけ変換
- `dangerouslySetInnerHTML` を廃止。テキストを区間に分けて React の `<mark>` で描画
- 429 / 409 / 失敗 / 生成中 を画面内に表示（`alert` をやめた）
- `status` が `generating` / `failed` の文書は一覧に出さない

### ハイライトの語境界

`\b` は英数字の隣でしか成立しない。実データには
...
のように記号で始まる項目があり、`\b\(all\)...` は一致しない。
語ごとに端が英数字かどうかを見て `\b` を付けるか決めるようにした。

### テスト

`src/logic/storyView.js` に純粋関数として切り出し、19件。

- 語境界（`together` の中の `gather` を拾わない）
- 長い語を優先（`in front` を `in` で切らない）
- 記号を含む語
- **HTMLを含む生成文が文字列のまま返る**
- 旧スキーマ3種の変換
- 分割して結合すると元の文章に戻る
...
- 実際の Gemini 呼び出しは未検証。Vertex AI と Translation API に繋がるため、
  エミュレータでは通せていない。ロジックの経路とスキーマまで。
- **未デプロイ。** 本番の Functions は今も旧実装（2回目が429で落ちる版）。

## フェーズ8: UI、アクセシビリティ、安定性（13.6のVite移行を除いて完了）

### 初期バンドル 766.17 kB → 185.9 kB

| 施策 | 効果 |
|---|---|
| 単語JSONを `public/data` からの遅延読み込みへ | **-475.94 kB** |
| ダッシュボード3画面を `React.lazy` で分割 | **-160.12 kB** |

`src/wordsData.json`（1.6MB）を import していたため、ログイン画面を開くだけで
全単語がバンドルに含まれていた。`src/logic/wordMaster.js` を追加して
必要になった時点で `fetch` し、セッション内は同じ Promise を使い回す。
`src/wordsData.json` は削除し、生成物は `public/data/` だけにした。

Chart.js は `AdminDashboard` からしか使わないので、遅延読み込みにしたことで
生徒が管理画面を開かない限り取りに行かなくなった。

ブラウザで確認: **ログイン画面で読み込まれるJSは `main` の1本だけ**。
単語データの取得は0回。
...
| 項目 | 変更前 | 変更後 |
|---|---|---|
| 送信 | `onClick` のみ | `<form onSubmit>`。**入力欄でEnterを押せる** |
| ラベル | `<label>` に `htmlFor` なし | `htmlFor` と `id` で紐付け |
| エラー | `alert()` | 画面内に `role="alert"` で表示 |
| エラー文言 | 一律「ログインに失敗しました」 | Firebase のコード別に案内を出し分け |
| 処理中 | ボタンのみ無効 | 入力欄も無効化 |
| PC表示 | カードが横いっぱいに伸びる | 最大幅420pxで中央寄せ |

ブラウザで確認済み: ラベル紐付けOK、Enterでログイン実行、
誤った資格情報で画面内にエラー表示、390 / 768 / 1440 のいずれも横スクロールなし。

### アクセシビリティ（13.3）

- `:focus-visible` のアウトラインを全ての操作要素に付けた（フォーカスリングを消さない）
- `.primary-action` / `.ghost-button` / `.motivation-option` / `.goal-chip` に `min-height: 44px`
- **選択状態を色だけで表さない。** `aria-pressed="true"` の項目に `✓` を付ける
- 単語力チェックにスワイプの代替ボタンを追加済み（フェーズ6）

### デバッグログ（13.4）

`console.log` が **179箇所**（`StudentDashboard` だけで85箇所）あり、
生徒名・ユーザーID・目標・回答履歴が本番コンソールへ流れていた。

`src/logic/logger.js` を追加し、全て `logger.debug` へ置き換えた。

- 本番ビルドでは `debug` / `info` は何も出さない
- `warn` / `error` は本番でも出す（障害の切り分けに要る）
- 手元で見たいときは `.env.local` に `REACT_APP_DEBUG_LOG=true`

### 未実施: 13.6 Vite移行

計画書のリスク表にも「機能修正後に独立工程として実施」とある。
バンドルは 185.9 kB まで落ちたので、13.7の完了条件は Vite なしで満たしている。
アプリ側の依存監査 69件 / critical 4 は `react-scripts` 5.0.1 由来なので、
これを解消したい場合は Vite 移行が要る。**別工程として残す。**

### 未確認

- 生徒画面・管理画面の実表示は未確認（ログインが必要）。
...
`node scripts/migrate-review-words.js --execute`

**実行前にバッチのバグを修正した。** commit 済みの `WriteBatch` を作り直さずに
使い回していたため、400件を超えるユーザー（最大502件）で確実に例外になる状態だった。
ドライランは書き込まないので発覚しなかった。
...
ドライランの予測と実行結果は完全に一致した。

最大のユーザー（旧502件）の実データを確認:

```
永続ID(w_)の文書 : 456
migratedTo 付き  : 480   ← 日次プランから除外される
実効の復習単語   : 476   ← 456（永続ID）+ 20（保留）
```

統合の例:

```
w_038676668a743646  about / 〜について(の)、〜に関して
  統合元: NVJ8yTDvMGPn4EgU2Ril, dZyzLuQUocCgDnGVJ4BK, eiken_words_2
```

Firestore のランダムIDが2件とインデックス由来IDが1件、**同じ単語で3重になっていた**。
これが1件に統合された。計画書9.6の「教材や級を切り替えても復習文書が重複しない」に対応する。

### 保留21件について

**対応不能の8件はマスターに単語自体が無い。**

```
fox / skin / stick / organize / stapler / eighteenth / turn into ~ / there is [are]~
```

Firestore の `textbooks/osaka-koukou-nyuushi/words` にしか存在しない単語で、
マスターを作る際の入力元に含めていなかった（`words.json` /
`public/words.json` / `highschool.json` の3つしか見ていない）。

データの形も違う。`partOfSpeech: "名, 動"`、`meaning: "杖、棒、棒状のもの; くっつく"`
のように品詞と意味が連結されており、別世代のデータと分かる。
**複数候補の13件**も、この形式差で「語+品詞+意味」が一致せず語だけの照合に落ちた結果。
誤った統合を避けるため、あえて書き込んでいない。

保留分は旧IDのまま残り、そのまま動作する。ただし将来その単語を語彙力チェックで
間違えると永続IDで別文書ができて二重になる。根本解決には
`textbooks/*/words` をマスターの入力元に加える作業が要る。**未着手。**

### 5. Firestore の単語をマスターへ取り込み、保留分を再移行

移行で保留になった21件を追跡したところ、**すべて `osaka-koukou-nyuushi` 由来**だった。
フェーズ4で正本を決めるとき、Firestore の `textbooks/*/words` を入力元に
含めていなかったのが原因。JSONの3ファイルしか見ていなかった。

#### Firestore の実態

| 教材 | Firestore | マスターに無い項目 | 語自体が無い |
|---|---:|---:|---:|
| `highschool-english` | 5,307 | 0 | 0 |
| `osaka-koukou-nyuushi` | **3,977** | **1,213** | 222 |
| `target-1900` | 1,900 | 337 | 158 |

- `public/words.json`（1,969件）は大阪府教材の**約半分しか無かった**。
  Firestore には 3,977 件ある。`pumpkin` / `firework` / `peach` など222語が
  マスターに存在しなかった。
- **`target-1900` という教材が存在する**が、アプリのどの画面からも参照されていない。
  取り込むと語彙力チェックの出題プールが変わりレベル判定に影響するため、
  今回は入れていない。**扱いは要判断。**

#### 対応

- `scripts/export-firestore-textbooks.js` を追加（読み取りのみ）
- `data-sources/firestore-textbooks.json` をビルド入力として追加
- `scripts/build-word-master.js` が Firestore 由来の単語も取り込むようにした
- 教材の収録範囲を JSON と Firestore の和集合にした。
  自由学習が読む `words-osaka.json` と、日次計画が読む Firestore のずれを解消

| | 変更前 | 変更後 |
|---|---:|---:|
| マスター件数 | 6,736 | **7,949** |
| `words-osaka.json` | 1,969 | **3,193** |
| `words-highschool.json` | 4,789 | 4,789 |

**既存6,736件のIDは1件も変わっていない**（消えたID 0 / 内容が変わったID 0 を確認）。
...
再実行の前に、**既に永続IDになっている文書を対象外にする修正**を入れた。
入れないと `migratedFrom`（統合元の記録）を自分自身のIDで上書きしてしまう。
実データで統合記録が残っていることを確認した。

```
w_038676668a743646 の migratedFrom:
  ["NVJ8yTDvMGPn4EgU2Ril", "dZyzLuQUocCgDnGVJ4BK", "eiken_words_2"]
```

最大ユーザーの最終状態: 実効の復習単語476件のうち **475件が永続ID**。

#### 残る1件

`heritage (名) 遺産` だけ保留のまま。マスターに
`[名] 遺産 L5` と `[名] 遺産 L6` の2件があり、どちらか決められないため。

同じ状況（語+品詞+意味が同じでレベルだけ違う）はマスター全体で5グループ10件ある。

```
despite [前] ～にもかかわらず   L5 / L6
elite    [名] エリート          L5 / L6
fatigue  [名] 疲労              L5 / L6
frequent [形] 頻繁な            L5 / L6
heritage [名] 遺産              L5 / L6
```

統合するとIDが1つ消えるので、既にそのIDを持つ復習文書があると孤立する。
5語のために踏む risk ではないと判断して残した。**要判断。**
