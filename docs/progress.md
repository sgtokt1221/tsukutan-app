...
| 指標 | 基準（フェーズ0） | フェーズ3後 | フェーズ4後 | フェーズ5後 | フェーズ6後 |
|---|---|---|---|---|---|
| `npm run build` | 成功・警告あり | 成功・警告1件 | 成功・警告1件 | 成功・警告1件 | 成功・警告1件 |
| `npm run lint` | **スクリプト未定義** | 警告1件・エラー0 | 警告1件・エラー0 | 警告1件・エラー0 | 警告1件・エラー0 |
| JSバンドル（gzip） | 766.17 kB | 768.54 kB | 822.1 kB（IDを付けた分。フェーズ8で除去） | 822.93 kB | 821.4 kB |
| ビルドの想定ホスト | `/tsukutan-app/` | `/` | `/` | `/` | `/` |
| アプリ側 `npm test` | 0件のため失敗 | **48件 成功** | 48件 成功 | **71件 成功** | **113件 成功** |
...
| 単語の永続ID | なし（配列index） | なし | **6,736件すべてに付与** | 6,736件 | 6,736件 |
| `src/VocabularyCheckTest.js` | 975行 | 975行 | 975行 | 975行 | **302行** |
| 5問目でのステージリセット | **発生する** | 発生する | 発生する | 発生する | **しない** |
| スクリプトのテスト | なし | なし | **13件 成功** | 13件 成功 | 13件 成功 |
| やる気レベルが日次計画に反映される | **されない** | されない | されない | **される** | される |
| 隣接レベル語の出題 | **常に0件（沈黙失敗）** | 同左 | 同左 | **動作** | 動作 |
