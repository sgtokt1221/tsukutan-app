...
describe('学習画面の中の案内', () => {
  ...
  it('**「使い方を見る」で、案内を全部もう一度出す**（ほかの保存は消さない）', () => {
    localStorage.setItem('tsukutan.coach.wordbook', '1');
    localStorage.setItem('tsukutan.coach.card.up.graduate', '1');
    localStorage.setItem('wordbook_progress_default_all', '12');
    resetAllCoaches();
    expect(localStorage.getItem('tsukutan.coach.wordbook')).toBeNull();
    expect(localStorage.getItem('tsukutan.coach.card.up.graduate')).toBeNull();
